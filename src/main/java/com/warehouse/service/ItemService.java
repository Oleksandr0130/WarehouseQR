...
public class ItemService {
    ...
    public String getQrCodeUrl(String id) {
        return qrCodeBaseUrl + id + ".png"; // Формирование URL (например, для отдачи внешним сервисам)
    }
    ...
    public Item addItem(Item item) {
        ...
        // Генерация QR-кода и сохранение его в объект
        generateQRCode(item);

        // Сохраняем товар с QR-кодом в базе данных
    ...
    private void generateQRCode(Item item) {
        try {
            // Генерация QR-кода
            QRCodeWriter qrCodeWriter = new QRCodeWriter();
            BitMatrix bitMatrix = qrCodeWriter.encode(item.getId(), BarcodeFormat.QR_CODE, 200, 200);

            // Конвертация в массив байтов (PNG)
            ByteArrayOutputStream pngOutputStream = new ByteArrayOutputStream();
            MatrixToImageWriter.writeToStream(bitMatrix, "PNG", pngOutputStream);
            byte[] qrCodeBytes = pngOutputStream.toByteArray();

            // Сохраняем массив байтов в объект
            item.setQrCodeImage(qrCodeBytes);

            // Также сохраняем URL для удобства интеграции
            item.setQrCodeUrl(getQrCodeUrl(item.getId()));
