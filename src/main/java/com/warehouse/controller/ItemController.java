...
public ResponseEntity<ItemDTO> addItem(@RequestBody ItemDTO itemDTO) {
    ...
    // Конвертируем массив байтов в Base64 для передачи клиенту
    String qrCodeBase64 = Base64.getEncoder().encodeToString(savedItem.getQrCode());

    // Преобразуем сохранённый объект в DTO
    ItemDTO responseDTO = itemMapper.toDTO(savedItem);
    responseDTO.setQrCode(qrCodeBase64); // Устанавливаем закодированный QR-код
